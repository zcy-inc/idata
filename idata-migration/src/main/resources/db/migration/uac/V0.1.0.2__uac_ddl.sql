create table if not exists idata.uac_user (
  ...
  sys_admin   smallint        not null default 0 comment '是否系统管理员(0:否,1、2:是)',
...
create table if not exists idata.uac_role (
  ...
  editor      varchar(20)     not null default '' comment '修改者',
  edit_time   datetime(3)     not null default current_timestamp(3) on update current_timestamp(3) comment '修改时间',
  role_code   varchar(20)     not null comment '角色编码', unique uk_role_code(role_code),
...
create table if not exists idata.uac_access (
  ...
  create_time datetime(3)     not null default current_timestamp(3) comment '创建时间',
  edit_time   datetime(3)     not null default current_timestamp(3) on update current_timestamp(3) comment '修改时间',
  access_code varchar(20)     not null comment '权限编码', unique uk_access_code(access_code),
  access_type varchar(20)     not null comment '权限类型',
  access_key  varchar(20)     comment '权限对应的ID或编码'
) engine=innodb auto_increment=20001 default charset=utf8mb4 comment='权限表';
...
create table if not exists idata.uac_role_access (
  ...
  edit_time   datetime(3)     not null default current_timestamp(3) on update current_timestamp(3) comment '修改时间',
  role_code   varchar(20)     not null comment '角色编码',
  access_code varchar(20)     not null comment '权限编码'
